	...
	class BG_Projects {
		...
		*/
		public function __construct() {
			...
			add_filter( 'postbox_classes_bg_projects_bg_projects_meta_boxes', array( $this,'_add_metabox_classes' ) );
			
			add_action('do_meta_boxes', array( $this , '_change_meta_boxes' ));

			add_filter('manage_bg_projects_posts_columns' , array( $this , '_add_post_type_column' ));
			
			add_action( 'manage_bg_projects_posts_custom_column' , array( $this , '_manage_post_type_column' ), 10, 2 );
			
		}

		function _build_post_type() {
			$custom_name = get_option('bg_projects_name_option');
			$name = (!empty($custom_name)) ? $custom_name : _x( 'Bears Projects', 'Post type name', 'bears-grid' );
			
			$custom_slug = get_option('bg_projects_slug_option');
			$slug = (!empty($custom_slug)) ? $custom_slug : 'bg-project';
			
			/** post type name filters **/
			$post_names = apply_filters( 'projects_post_type_name_filters',
				array(
					'singular' => _x( 'Project', 'Post type general name', 'bears-grid' ),
					'plural'   => _x( 'Projects', 'Post type plural name', 'bears-grid' ),
					'name'   => $name,
				)
			);

			/** icon filters **/
			$menu_icon = apply_filters( 'bg_projects_menu_icon_filters','dashicons-index-card');
			...
			register_post_type( 'bg_projects', $args );
		}
		
		function _build_taxonomies() {
			register_taxonomy('bg_projects_category', 'bg_projects', 
				array(
					'labels' => array(
		...
		function _change_meta_boxes() {
			remove_meta_box( 'postimagediv', 'bg_projects', 'side' );
			
			add_meta_box( 'postimagediv', __( 'Cover Image', 'bears-grid' ), 'post_thumbnail_meta_box', 'bg_projects', 'side' );
		}
		...
		function _add_post_type_column( $columns ) {
			...
			$columns['category'] = __( 'Projects Categories', 'bears-grid' );
		...
		function _manage_post_type_column( $column, $post_id  ) {
			...
			switch ( $column ) {
				...
				case 'category' :
					 $terms = get_the_term_list( $post_id , 'bg_projects_category' , '' , ', ' , '' );
