...
foreach( $datas as $_data ){
	...
	$a_height = array( ( $height*2 + $space ), $height*1.5, $height );
	$rh = array_rand( $a_height, 1 );
	$_height = $a_height[$rh] . $unit;
	
	/* $width = '(100% - '.(($column-1)*$_space).$unit.') / '.$column;
	$a_width = array( ( $width.'*2' ), $width );
	$rw = array_rand( $a_width, 1 );
	$_width = 'calc('.$a_width[$rw].')';
	
	$_data['style'] .= '; width: ' . $_width; */
			...
			<div class="<?php echo $class; ?>" style="<?php echo $_data['style']; ?>">
				<div class="bg-item-inner" style="height: <?php echo $_height; ?>;">
					...
					<div class="info">
						...
						<div class="description"><?php echo $_data['content']; ?></div>
					</div>
