<?php
class BGVC_Porfolio_Element extends WPBakeryShortCode {
    private $base_name = 'bgvc_porfolio_element';
    function __construct () {
        $this->settings['base'] = $this->base_name;
        add_action( 'init', array( $this, 'vc_mapping' ) );
        add_shortcode( 'bgvc_porfolio_element', array( $this, 'vc_output' ) );
    }
    /**
     * @since 1.0.0
     */
    public function vc_mapping () {
        ...
        vc_map(
        array(
            'name' => __('Porfolio Element', 'bears-grid'),
            'base' => $this->base_name,
            'description' => __('Porfolio Element.', 'bears-grid'),
            'category' => __('Bears', 'bears-grid'),
            'icon' => BG_INC_URL . '/vc-elements/portfolio/icon.png',
            'params' => array(
                array(
                    'type' => 'el_id',
    ...
    public function vc_output ( $atts, $content ) {
		...
		$filepath = apply_filters('_bears_grid_portfolio_element_layout', $filepath);
		
		return _render_view( $filepath, $atts, true );
    }
}
new BGVC_Porfolio_Element();