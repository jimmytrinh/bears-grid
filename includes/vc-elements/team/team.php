<?php
class BGVC_Team_Element extends WPBakeryShortCode {
    private $base_name = 'bgvc_team_element';
    function __construct () {
        $this->settings['base'] = $this->base_name;
        add_action( 'init', array( $this, 'vc_mapping' ) );
        add_shortcode( 'bgvc_team_element', array( $this, 'vc_output' ) );
    }
    /**
     * @since 1.0.0
     */
    public function vc_mapping () {
        ...
        vc_map(
        array(
            'name' => __('Team Element', 'bears-grid'),
            'base' => $this->base_name,
            'description' => __('Team Element.', 'bears-grid'),
            'category' => __('Bears', 'bears-grid'),
            'icon' => BG_INC_URL . '/vc-elements/team/icon.png',
            'params' => array(
                array(
                    'type' => 'el_id',
    ...
    public function vc_output ( $atts, $content ) {
		...
		$filepath = apply_filters('_bears_grid_team_element_layout', $filepath);
		
		return _render_view( $filepath, $atts, true );
    }
}
new BGVC_Team_Element();